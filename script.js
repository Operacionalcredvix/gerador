...
async function initializeSupabase() {
  try {
    ...
    if (testError && testError.code !== "PGRST116") {
      // PGRST116 = tabela vazia, o que é aceitável
      throw testError
    }
...
function generatePassword(elements, charSets) {
  try {
    let charSet = ""
    if (elements.optionsCheckboxes.includeUppercase.checked)
      charSet += charSets.uppercase
    if (elements.optionsCheckboxes.includeLowercase.checked)
      charSet += charSets.lowercase
    if (elements.optionsCheckboxes.includeNumbers.checked)
      charSet += charSets.numbers
    if (elements.optionsCheckboxes.includeSymbols.checked)
...
async function getPasswordStats(elements) {
  ...
  try {
    const localStats =
      JSON.parse(localStorage.getItem("passwordStats")) || stats
...
async function incrementCounters(elements) {
  ...
  try {
    ...
    if (record && record.id) {
      ...
      const newTodayCount =
        currentServerDate === today ? currentStats.today_count + 1 : 1
    ...
    elements.copyButton.addEventListener("click", async () => {
      try {
        ...
        if (
          ...
          password === 'Selecione as opções e clique em "Gerar Nova Senha"'
        )
          return
...
async function initializeApp() {
  try {
    ...
    elements.passwordDisplay.value =
      'Selecione as opções e clique em "Gerar Nova Senha"'
    updateStrengthIndicator("", elements)
  } catch (error) {
...
// --- Função para configurar os Modais Legais ---
...
// Dentro da sua função async function initializeApp(), adicione a chamada para a nova função:
async function initializeApp() {
  try {
    // ... (todo o seu código existente dentro de initializeApp)

    setupEventListeners(elements, charSets)
    setupFAQ()
    setupSocialSharing()
    updateCopyrightYear()
    setupLegalModals() // <-- ADICIONE ESTA LINHA
